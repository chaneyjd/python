...
class CalendarExample:
  ...
  def _PrintAllEventsOnDefaultCalendar(self):
    ...
    for i, an_event in zip(xrange(len(feed.entry)), feed.entry):
      print '\t%s. %s %s' % (i, an_event.title.text,an_event.published,)
  ...
  def Run(self, delete='false'):
    ...
    # see_u_ext_prop = self._AddExtendedProperty(see_u_reminder, name='propname', value='propvalue')
    ...
    # (quick_add_event, see_u_ext_prop) = self._batchRequest(see_u_ext_prop, quick_add_event)
